...
CLOUDINARY_URL     = os.environ.get("CLOUDINARY_URL")  # format: cloudinary://key:secret@cloud
...
def upload_video_cloudinary():
    log("☁️ رفع الفيديو على Cloudinary...")
    try:
        ...
        log(f"❌ خطأ Cloudinary: {e}", "error")
        return None

def upload_image_cloudinary():
    log("☁️ رفع الصورة على Cloudinary...")
    try:
        result = cloudinary.uploader.upload(
            "temp_image.jpg",
            folder="radar_news",
            overwrite=True,
            public_id=f"img_{int(time.time())}"
        )
        url = result.get("secure_url")
        log(f"✅ الصورة منشورة: {url[:60]}...")
        return url
    except Exception as e:
        log(f"❌ خطأ Cloudinary صورة: {e}", "error")
        return None
...
def job():
    ...
    try:
        ...
        if post_type == "reel":
            ...
            if not generate_voice(content["script"]):
                ...
                if not create_reel():
                    ...
                    # 5. Upload video
                    video_url = upload_video_cloudinary()
                    if not video_url:
        ...
        # Image post path
        img_url = upload_image_cloudinary()
        if not img_url:
...
def api_status():
    return jsonify({
        ...
        "cloudinary_connected": bool(CLOUDINARY_URL),
